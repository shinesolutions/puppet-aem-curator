define aem_curator::install_aem65_sp23(
  $aem_license_base,
  $aem_artifacts_base,
  ...
  aem_curator::install_aem65 { "${aem_id}: Install AEM":
    ...
    aem_id                  => $aem_id,
    aem_type                => $aem_type,
  } -> aem_curator::install_aem_package { "${aem_id}: Install service pack 23":
    tmp_dir         => $tmp_dir,
    file_name       => 'aem-service-pkg-6.5.23.0.zip',
    package_name    => 'aem-service-pkg',
    package_group   => 'adobe/cq650/servicepack',
    package_version => '6.5.23.0',
